  ...
  <div class="win-his-header-wrapper">
    <div class="win-his-header">
      <div class="win-his-header--logo-wrapper">
        ...
        <el-popover placement="bottom" ref="application-select" width="220" trigger="click"
          popper-class="application-popover">
          ...
          <span slot="reference" class="application-name">
        ...
        <div class="menu-list">
          <scroll-pane ref="scrollPane" class="scroll-view-wrapper" style="height: 100%;">
            <div class="router-link-wrapper">
              <router-link v-for="route in routes" ref="route" :key="route.id" class="nav-wrap" active-class="active"
                :to="route.path">
                <div class="nav-item">
                  <div class="nav-content">
                    <span>{{ route.name }}</span>
                  </div>
                  <i class="right-icon el-icon-close" @click.stop.prevent="handleRemoveMenu(route)"></i>
                </div>
              </router-link>
              <el-popover placement="bottom" width="200" trigger="click" ref="more-menu-popover"
                @hide="()=>moreMenuListVisible=false" @show="()=>moreMenuListVisible=true"
                ...
                <div class="more-container">
                  ...
                  <el-input placeholder="请输入关键字检索" size="mini" class="search" suffix-icon="el-icon-search"
                    v-model="keyword" />
                  <div style="height: 300px;">
                    <el-scrollbar style="height: 100%;">
                      <el-tree ref="tree" :data="applicationTree" :filter-node-method="filterNode"
                ...
                <span slot="reference" class="more-menu">
                  <span class="text">
                    <span>更多</span>
...
<script>
  ...
  export default {
    ...
    watch: {
      ...
      $route (val) {
        const { path } = val
        this.$store.dispatch('checkAppChange', { path, $router: this.$router })
    ...
    methods: {
      ...
      handleNodeClick (data) {
        ...
        if (!data.path) {
          this.$message.warning('当前菜单未配置连接地址')
          return
        }
